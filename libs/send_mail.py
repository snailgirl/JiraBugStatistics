...
sfile_path = os.path.join(sys_path, 'conf', 'mail.ini')

class SendMail:
    def __init__(self, sfile=sfile_path, param="gmail"):
        """
        初始化ini配置文件信息
    ...
    def __get_mail_conf(self):
        ...
        conf.read(self.__sfile,encoding='utf-8')
        try:
            mail_host = conf.get(self.__param, "host")
            mail_port = conf.get(self.__param, "port")
            mail_user = conf.get(self.__param, "user")
            mail_passwd_base64 = conf.get(self.__param, "passwd")
            # mail_passwd = base64.decodestring(mail_passwd_base64) #qq邮箱不需要
            sender_name = conf.get(self.__param, "name")
            sender_addr = conf.get(self.__param, "sender")
        except Exception as e:
            print ("Error:", e)
            sys.exit(-2)
        return mail_host, mail_port,mail_user, mail_passwd_base64, sender_name, sender_addr
    ...
    def send_mail(self, rcpt_list, subject, content, att_list=None, cc_list=None, mail_type="html"):
        ...
        mail_host,mail_port, mail_user, mail_passwd, sender_name, sender_addr = self.__get_mail_conf()
        # 格式化邮箱地址
...
if __name__ == '__main__':
    ...
    status = send_mail_obj.send_mail(['tester@123.com'], '测试', '测试内容')